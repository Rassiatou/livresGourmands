// controllers/panier.controller.js
import { pool } from "../db.js";

// GET /api/panier-items?panierId=1  OU  /api/panier-items?clientId=3
export async function listItems(req, res) {
  try {
    ...
    // Si on n'a pas de panierId mais qu'on a un clientId → on récupère le panier actif
    ...
    const [items] = await pool.query(
      `SELECT 
       ...
       JOIN ouvrages o ON o.id = pi.ouvrage_idOuvrage
...
export async function addItem(req, res) {
  try {
    ...
    const [[exist]] = await pool.query(
      `SELECT * 
       FROM panier_items 
       WHERE panier_idPanier = ? 
         AND ouvrage_idOuvrage = ?`,
    ...
    if (exist) {
      ...
      await pool.query(
        `INSERT INTO panier_items 
           (panier_idPanier, ouvrage_idOuvrage, quantite, prix_unitaire)
         VALUES (?,?,?,?)`,
        [
          panier.idPanier,
          Number(ouvrage_idOuvrage),
